...
When the user chooses to edit a journal entry, the first step in the process is for the client to populate the form with the current values that are stored in the database. Then, when the user has made all of the changes that she wants to, and clicks the "Save" button, a PUT request will be sent to your server.

1. Create an `update_entry` function to contain the logic.
1. In the `update_entry` function write an `UPDATE JournalEntries` SQL statement to replace the data in the database with the information sent from the client.
